...
namespace {
    ...
    /** helleniccoin: blocks that are waiting to be processed, the key points to previous CBlockIndex entry */
    struct WaitElement {
...
void static ProcessGetBlockData(CNode* pfrom, const CChainParams& chainparams, const CInv& inv, CConnman* connman)
{
    ...
    if (send && (pindex->nStatus & BLOCK_HAVE_DATA))
    {
        ...
        if (inv.hash == pfrom->hashContinue)
        {
            ...
            // helleniccoin: send latest proof-of-work block to allow the
...
bool static ProcessHeadersMessage(CNode* pfrom, CConnman* connman, CTxMemPool& mempool, const std::vector<CBlockHeader>& headers, const CChainParams& chainparams, bool via_compact_block)
{
...

    {
        ...
        if (nCount == MAX_HEADERS_RESULTS) {
            ...
            connman->PushMessage(pfrom, msgMaker.Make(NetMsgType::GETBLOCKS, ::ChainActive().GetLocator(pindexLast), uint256()));
        }
...
bool ProcessMessage(CNode* pfrom, const std::string& msg_type, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CTxMemPool& mempool, CConnman* connman, BanMan* banman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::VERSION) {
        ...
        // helleniccoin: relay sync-checkpoint
        {
            LOCK(cs_main);
...
#ifdef ENABLE_CHECKPOINTS
        // helleniccoin: ask for pending sync-checkpoint if any
    ...
    // helleniccoin: set/unset network serialization mode for new clients
    ...
    if (msg_type == NetMsgType::INV) {
        ...
        if (best_block != nullptr) {
            connman->PushMessage(pfrom, msgMaker.Make(NetMsgType::GETBLOCKS, ::ChainActive().GetLocator(pindexBestHeader), *best_block));
