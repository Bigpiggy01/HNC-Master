...
static void SetupCliArgs()
{
    ...
    gArgs.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to helleniccoind). This changes the RPC endpoint used, e.g. http://127.0.0.1:9902/wallet/<walletname>", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  helleniccoin-cli [options] <command> [params]  Send command to " PACKAGE_NAME "\n"
                "or:     helleniccoin-cli [options] -named <command> [name=value]...  Send command to " PACKAGE_NAME " (with named arguments)\n"
                "or:     helleniccoin-cli [options] help                List commands\n"
                "or:     helleniccoin-cli [options] help <command>      Get help for a command\n";
