...
static const bool DEFAULT_TXINDEX = true;  // helleniccoin: txindex is required for PoS calculations (might change in the future)
...
// helleniccoin:
CAmount GetProofOfWorkReward(int nHeight, int64_t nFees = 0);
CAmount GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees);
bool GetCoinAge(const CTransaction& tx, const CCoinsViewCache &view, uint64_t& nCoinAge, bool isTrueCoinAge = true); // helleniccoin: get transaction coin age
